...
@section('container')
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3 rtl:space-x-reverse">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="ms-1 text-sm font-medium text-gray-700 md:ms-2 dark:text-gray-400">Loker</span>
                </div>
            </li>
            <li aria-current="page">
                <div class="flex items-center">
    ...
    <div class="bg-white p-4 rounded-lg">
        <form id="createForm" action="{{ route('partners.jobAplicants.store') }}" method="post">
            @csrf
            <div class="mb-2 grid gap-4 sm:grid-cols-2">
                <div>
                  ...
                  @enderror"
                        value="{{ old('job_title') }}" placeholder="Nama Loker" autocomplete="off" required>
                    @error('job_title')
                ...
                <div>
                    ...
                    <select id="type_job" name="type_job"
                        ...
                        <option disabled {{ old('type_job') == null ? 'selected' : '' }}>--Jenis Loker--</option>
                        <option value="WFO" {{ old('type_job') == 'WFO' ? 'selected' : '' }}>WFO</option>
                        <option value="WFH" {{ old('type_job') == 'WFH' ? 'selected' : '' }}>WFH</option>
                        <option value="Hybrid" {{ old('type_job') == 'Hybrid' ? 'selected' : '' }}>Hybrid</option>
                    </select>
                </div>
                <div>
                    ...
                    <select id="category_id" name="category_id"
                        ...
                        @foreach ($categories as $categorie)
                            @if (old('category_id') == $categorie->id)
                                <option value="{{ $categorie->id }}" selected>{{ $categorie->name_category }}</option>
                            @else
                ...
                <div>
                  ...
                  @enderror"
                        value="{{ old('location') }}" placeholder="Lokasi Loker" autocomplete="off" required>
                    @error('location')
                        <div class="invalid-feedback text-gray-900 dark:text-white">
                            {{ $message }}
                        </div>
                ...
                <div class="relative max-w">
                    ...
                    <input datepicker datepicker-format="dd/MM/yyyy" type="text" autocomplete="off" id="date_posted"
                        name="date_posted"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500 @error('date_posted') is-invalid
                  @enderror"
                        placeholder="Pilih Tanggal" value="{{ old('date_posted') }}" required>
                </div>
                <div class="relative max-w">
                    ...
                    <input datepicker datepicker-format="dd/MM/yyyy" type="text" autocomplete="off" id="date_closing"
                        name="date_closing"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500 @error('date_closing') is-invalid
                  @enderror"
                        placeholder="Pilih Tanggal" value="{{ old('date_closing') }}" required>
                </div>
                <div>
                  ...
                  @enderror"
                        value="{{ old('salary_min') }}" placeholder="Min. Gaji" autocomplete="off" required>
                    @error('salary_min')
                        <div class="invalid-feedback text-gray-900 dark:text-white">
                            {{ $message }}
                        </div>
                ...
                <div>
                    <label for="salary_max" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Max.
                        Gaji (Opsional, Isi Jika Ada Range Gaji.)</label>
                  ...
                  @enderror"
                        value="{{ old('salary_max') }}" placeholder="Max. Gaji" autocomplete="off" required>
                    @error('salary_max')
                        <div class="invalid-feedback text-gray-900 dark:text-white">
                            {{ $message }}
                        </div>
            ...
            <div class="mb-4">
                ...
                <textarea id="job_description" name="job_description" rows="4"
                    ...
                    placeholder="Tulis keterangan tentang loker..."></textarea>
            </div>
